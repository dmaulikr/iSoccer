...
@property (copy) void (^onComponent)(NSString *,NSInteger);
@property (copy) void (^onHiddenCompletion)(void);
...
//接收array必须包含多维;
+(void)showPickerViewInViewComponents:(UIView *)view
                          withStrings:(NSArray *)strings
                          withOptions:(NSDictionary *)options
                       withComponents:(NSMutableArray*)components
                           completion:(void (^)(NSString *,NSInteger))completion
                               hedden:(void(^)(void))hedden{
    [[self sharedView] initializePickerViewInView:view
                                        withArray:strings
                                      withOptions:options isComponent:YES components:components];
    
    [[self sharedView] setPickerHidden:NO];
    [self sharedView].onComponent = completion;
    [self sharedView].onHiddenCompletion = hedden;
...
+(void)showPickerViewInView:(UIView *)view
  ...
  [[self sharedView] initializePickerViewInView:view
                                      withArray:strings
                                    withOptions:options isComponent:NO components:@[]];
...
-(void)initializePickerViewInView: (UIView *)view
                      ...
                      withOptions: (NSDictionary *)options isComponent:(BOOL)isComp components:(NSArray*)components{
  ...
  _pickerContainerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, _pickerViewContainerView.bounds.size.height - 260.0, _pickerViewContainerView.frame.size.width, 260.0)];
  ...
  //Add pickerView
  _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0.0, 44.0, _pickerViewContainerView.frame.size.width, 216.0)];
  ...
  //
    ...
    if(components.count > 0 && isComp == YES)
    {
        for(NSInteger i = 0;i < components.count;i++)
        {
            NSDictionary * data = components[i];
            
            NSString * componentString = [data objectForKey:@"component"];
            NSString * rowString = [data objectForKey:@"row"];
            
            [_pickerView  selectRow:rowString.integerValue inComponent:componentString.integerValue animated:NO];
        }
    }else{
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent: (NSInteger)component {
    if(hasComponent)
    {
        NSMutableArray * data = _pickerViewArray[component];
...
- (NSString *)pickerView: (UIPickerView *)pickerView
    ...
    if(hasComponent)
    {
        return _pickerViewArray[component][row];
    }
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if(hasComponent)
    {
        self.onComponent([[_pickerViewArray objectAtIndex:component] objectAtIndex:row],component);
        return;
    }
...
if(hasComponent)
{
    [pickerViewLabel setText:[NSString stringWithFormat:@"%@",_pickerViewArray[component][row]]];
