...
- (void)viewDidLoad {
    ...
    canUseMoneyText.text = [NSString stringWithFormat:@"%@元",canUseMoney];
}
    ...
    dispatch_source_set_event_handler(_timer, ^{
        if(timeout<=0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [button setTitle:@"重发送验证码" forState:UIControlStateNormal];
                button.userInteractionEnabled = YES;
            });
        }else{
            dispatch_async(dispatch_get_main_queue(), ^{
                
                [button setTitle:[NSString stringWithFormat:@"%zd秒重新发送",timeout] forState:UIControlStateNormal];
